...
## 2026-08-14 - Accessible Floating Action Widgets
**Learning:** We observed that floating interactive widgets (like the WhatsApp chat toggle) often lack proper semantic relationships with their content panels. Without `aria-expanded` and `aria-controls` on the trigger, and `role="region"` with a label on the target panel, screen readers cannot properly associate the toggle action with the opening of the content panel. Additionally, relying solely on CSS transforms for hover states without providing a `focus-visible` state makes these widgets effectively invisible to keyboard users.
**Action:** When implementing floating action buttons that reveal content (like chat widgets, help toggles, or floating menus), always establish a clear semantic relationship using `aria-controls` linked to the panel's `id`, manage state with `aria-expanded`, ensure the panel has a structural role like `role="region"` with an `aria-label`, and provide explicit visual focus states using `focus-visible` utility classes.
## 2024-05-18 - Native Form Submissions for A11y
**Learning:** React state-driven UIs often use `onClick` handlers on buttons instead of wrapping inputs in `<form>` elements. This breaks default keyboard accessibility, specifically preventing users from pressing "Enter" to submit a form.
**Action:** When creating forms like checkout or login flows, always wrap inputs in a `<form>` tag with an `onSubmit` handler (calling `e.preventDefault()`), and ensure the primary action button has `type="submit"` instead of an `onClick`.
